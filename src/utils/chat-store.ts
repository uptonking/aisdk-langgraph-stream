...
export async function createChat(): Promise<string> {
  // const id = generateId();
  const id = `chat${new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().replaceAll('-', '').replaceAll(':', '').replaceAll('.', '')}`;
  getChatFile(id);
  return id;
}

/**
 * persist chat `messages` to file/db
 */
...
/**
 * read or create a file for chat id
 *
...
/** read or create a file for chat id and return its content */
